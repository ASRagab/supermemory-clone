...
describe('MemoryExtensionDetectorService', () => {
  ...
  describe('Heuristic Detection', () => {
    it('should detect extension with indicators', async () => {
      ...
      const newMem = createMemory('I like programming, especially functional programming with Haskell', 'new')
    ...
    it('should not detect extension if new is substring of old', async () => {
      ...
      const existing = createMemory('I like programming, especially functional programming', 'old')
    ...
    it('should detect extension with more detail', async () => {
      ...
      const newMem = createMemory('For data science, I use Python and web development tasks', 'new')
  ...
  describe('LLM Detection', () => {
    it('should use LLM when available', async () => {
      ...
      const mockProvider = createMockProvider({
        mockJsonResponses: [{ isExtension: true, confidence: 0.85, reason: 'Adds specific use cases' }],
      })
  ...
  describe('Statistics', () => {
    it('should track detection statistics', async () => {
      ...
      await detector.checkExtension(createMemory('For data science, I use Python with ML tools', 'new1'), existing)
      await detector.checkExtension(createMemory('For data science, I use Python with ML tools', 'new1'), existing) // Cache
    ...
    it('should track extension rate', async () => {
      ...
      // Extension
      await detector.checkExtension(createMemory('I like programming with TypeScript and Python', 'new1'), existing)
  ...
  describe('Edge Cases', () => {
    ...
    it('should handle very long content', async () => {
      ...
      const longContent =
        'I use Python for many things including '.repeat(100) + 'data science, web development, automation'

      const result = await detector.checkExtension(createMemory(longContent, 'id1'), createMemory(shortContent, 'id2'))
    ...
    it('should handle extension indicators', async () => {
      ...
      const existing = createMemory('I like programming', 'old')
      const newMem = createMemory('I like programming. Additionally, I enjoy functional programming', 'new')
