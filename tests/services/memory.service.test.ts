...
import { MemoryService, createMemoryService, resetMemoryService } from '../../src/services/memory.service'
...
describe('MemoryService', () => {
  ...
  describe('feature flag defaults', () => {
    ...
    it('should use regex extraction when LLM flag is disabled', async () => {
      ...
      const memories = await localService.extractMemories('Paris is the capital of France.')
  ...
  describe('detectRelationships', () => {
    it('should detect relationships between new and existing memories', () => {
      ...
      const existingMemories: Memory[] = [createMockMemory('The API uses REST architecture.', 'mem1')]
    ...
    it('should return array of relationships', () => {
      ...
      const existingMemories: Memory[] = [createMockMemory('The project uses version 1.0.', 'mem1')]
    ...
    it('should include confidence scores when relationships detected', () => {
      ...
      const existingMemories: Memory[] = [createMockMemory('The API returns JSON responses.', 'mem1')]
    ...
    it('should include relationship type', () => {
      ...
      const existingMemories: Memory[] = [createMockMemory('The API handles requests with caching.', randomUUID())]

      const relationships = service.detectRelationships(newMemory, existingMemories)

      for (const rel of relationships) {
        expect(['updates', 'extends', 'derives', 'contradicts', 'related', 'supersedes']).toContain(rel.type)
      }
    })
  })
  ...
  describe('updateIsLatest', () => {
    ...
    it('should potentially set supersededBy on old memory', () => {
      ...
      expect(existingMemory.supersededBy === 'new-mem' || existingMemory.supersededBy === undefined).toBe(true)
    })
    ...
    it('should supersede highly similar content when similarity is very high', () => {
      ...
      const existingMemory = createMockMemory('The database uses PostgreSQL version 13 for all storage.', 'old-mem')
      const newMemory = createMockMemory('The database uses PostgreSQL version 13 for all storage needs.', 'new-mem')
  ...
  describe('relationship detection path selection', () => {
    it('should route to embedding detector when embeddings flag is enabled', async () => {
      ...
      const { createMemoryService, resetMemoryService } = await import('../../src/services/memory.service')
      const { createMemoryRepository, resetMemoryRepository } = await import('../../src/services/memory.repository')
  ...
  describe('processAndStoreMemories partial failures', () => {
    it('should roll back when relationship storage fails', async () => {
      ...
      vi.spyOn(isolatedRepo, 'createRelationshipBatch').mockRejectedValue(new Error('relationship write failed'))

      await expect(
        localService.processAndStoreMemories('The deadline is Friday. Update: The deadline is Friday now.')
    ...
    it('should roll back when supersede update fails', async () => {
      ...
      vi.spyOn(isolatedRepo, 'markSuperseded').mockRejectedValue(new Error('supersede failed'))
