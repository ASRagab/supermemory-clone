...
describe('EnhancedMemoryService feature flags', () => {
  ...
  it('should disable embedding detection by default', async () => {
    ...
    const { DEFAULT_ENHANCED_CONFIG } = await import('../../../src/services/relationships/memory-integration.js')
  ...
  it('should not supersede across container tags', async () => {
    ...
    const { createEnhancedMemoryService } = await import('../../../src/services/relationships/memory-integration.js')
...
function createMockMemory(content: string, id: string, overrides: Partial<Memory> = {}): Memory {
  return {
    id,
    content,
    type: 'fact',
