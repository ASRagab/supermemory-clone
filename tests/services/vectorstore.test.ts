 ...
 */
function createTestEntry(id: string, embedding?: number[], metadata?: Record<string, unknown>): VectorEntry {
  return {
    id,
...
describe('InMemoryVectorStore', () => {
  ...
  describe('CRUD operations', () => {
    ...
    it('should batch add entries', async () => {
      const entries = [createTestEntry('test-1'), createTestEntry('test-2'), createTestEntry('test-3')]
  ...
  describe('search', () => {
    ...
    it('should filter by metadata', async () => {
      await store.add(createTestEntry('tagged', normalizeVector([0.9, 0.1, 0.0, 0.0]), { tag: 'special' }))
...
describe.skip('SQLiteVSSStore (REMOVED - unused implementation)', () => {
  ...
  describe('CRUD operations', () => {
    ...
    it('should batch add with transaction', async () => {
      const entries = Array.from({ length: 100 }, (_, i) => createTestEntry(`test-${i}`, createRandomEmbedding()))
  ...
  describe('search', () => {
    ...
    it('should filter by metadata', async () => {
      await store.add(createTestEntry('tagged', normalizeVector([0.9, 0.1, 0.0, 0.0]), { category: 'special' }))
