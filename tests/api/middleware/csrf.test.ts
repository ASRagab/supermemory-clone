...
function csrfMiddleware(config: CsrfConfig) {
  ...
  return async (c: Context, next: Next) => {
    ...
    if (!cookieToken || !headerToken) {
      return c.json({ error: { code: 'CSRF_TOKEN_MISSING', message: 'CSRF token missing' } }, 403)
    }
    ...
    if (!parsedCookie || !parsedHeader) {
      return c.json({ error: { code: 'CSRF_TOKEN_INVALID', message: 'CSRF token malformed' } }, 403)
    }

    // Verify signatures
    const cookieValid = verifyToken(parsedCookie.value, parsedCookie.signature, parsedCookie.timestamp, secret)
    const headerValid = verifyToken(parsedHeader.value, parsedHeader.signature, parsedHeader.timestamp, secret)

    if (!cookieValid || !headerValid) {
      return c.json({ error: { code: 'CSRF_TOKEN_INVALID', message: 'CSRF token invalid or expired' } }, 403)
    }

    // Double-submit cookie pattern: tokens must match
    if (parsedCookie.value !== parsedHeader.value) {
      return c.json({ error: { code: 'CSRF_TOKEN_MISMATCH', message: 'CSRF token mismatch' } }, 403)
    }
    ...
    if (checkOrigin) {
      ...
      if (origin && !validateOrigin(origin, host)) {
        return c.json({ error: { code: 'ORIGIN_MISMATCH', message: 'Origin validation failed' } }, 403)
      }
    }
    ...
    if (checkReferer) {
      ...
      if (referer && !validateReferer(referer, host)) {
        return c.json({ error: { code: 'REFERER_MISMATCH', message: 'Referer validation failed' } }, 403)
      }
    }
