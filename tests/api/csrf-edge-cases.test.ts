...
describe('Expired Token Edge Cases', () => {
  ...
  it('should reject token from far future', async () => {
    ...
    const signature = createHmac('sha256', secret).update(`${value}.${futureTimestamp}`).digest('hex')
  ...
  it('should handle token at exact expiry boundary', async () => {
    ...
    const value = randomBytes(32).toString('hex')
    const signature = createHmac('sha256', secret).update(`${value}.${expiryTime}`).digest('hex')
  ...
  it('should handle very large timestamp', async () => {
    ...
    const value = randomBytes(32).toString('hex')
    const signature = createHmac('sha256', secret).update(`${value}.${largeTimestamp}`).digest('hex')
