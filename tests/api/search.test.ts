  ...
  private hybridSearch(docs: Document[], query: string, threshold: number): SearchResult[] {
    ...
    fulltextResults.forEach((result, index) => {
      ...
      if (existing) {
        ...
        // Merge highlights
        const allHighlights = new Set([...(existing.result.highlights || []), ...(result.highlights || [])])
...
describe('Search API', () => {
  ...
  describe('POST /search (basic search)', () => {
    ...
    it('should return results sorted by score descending', async () => {
      ...
      for (let i = 1; i < response.data.results.length; i++) {
        expect(response.data.results[i - 1]!.score).toBeGreaterThanOrEqual(response.data.results[i]!.score)
      }
    })
  })
    ...
    describe('rerank parameter', () => {
      ...
      it('should boost exact phrase matches', async () => {
        ...
        if (response.data.results.length > 0) {
          const hasMatch = response.data.results.some((r) => r.content.toLowerCase().includes('static typing'))
          if (hasMatch) {
  ...
  describe('Search Highlights', () => {
    it('should include highlights for matching terms', async () => {
      ...
      const resultWithHighlights = response.data.results.find((r) => r.highlights && r.highlights.length > 0)
    ...
    it('should limit highlights to 3 per result', async () => {
      api.addDocument({
        id: 'long-doc',
        content: 'Test sentence one. Test sentence two. Test sentence three. Test sentence four. Test sentence five.',
