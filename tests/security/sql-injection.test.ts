 ...
 */
const safeSQLInputSchema = z
  .string()
  .refine((val) => !containsSQLInjection(val), { message: 'Input contains potential SQL injection patterns' })
...
describe('Time-based Blind SQL Injection Detection', () => {
  ...
  it('should detect SLEEP with various durations', () => {
    const payloads = ["' AND SLEEP(1)--", "' AND SLEEP(10)--", "' AND SLEEP(0)--"]
...
describe('Parameterized Query Safety', () => {
  ...
  describe('Safe Query Patterns', () => {
    it('should use parameterized queries for user input', async () => {
      ...
      expect(db.query).toHaveBeenCalledWith('SELECT * FROM memories WHERE content = $1', [userInput])
    })
    ...
    it('should use multiple parameters safely', async () => {
      ...
      await db.query('SELECT * FROM memories WHERE user_id = $1 AND container_tag = $2', [userId, containerTag])

      expect(db.query).toHaveBeenCalledWith('SELECT * FROM memories WHERE user_id = $1 AND container_tag = $2', [
        userId,
        containerTag,
      ])
    })
    ...
    it('should parameterize INSERT statements', async () => {
      ...
      await db.execute('INSERT INTO memories (content, user_id) VALUES ($1, $2)', [maliciousContent, 'user123'])

      expect(db.execute).toHaveBeenCalledWith('INSERT INTO memories (content, user_id) VALUES ($1, $2)', [
        maliciousContent,
        'user123',
      ])
    })
    ...
    it('should parameterize UPDATE statements', async () => {
      ...
      await db.execute('UPDATE memories SET container_tag = $1 WHERE id = $2', [maliciousTag, 'memory-123'])

      expect(db.execute).toHaveBeenCalledWith('UPDATE memories SET container_tag = $1 WHERE id = $2', [
        maliciousTag,
        'memory-123',
      ])
    })
    ...
    it('should parameterize DELETE statements', async () => {
      ...
      await db.execute('DELETE FROM memories WHERE id = $1', [maliciousId])

      expect(db.execute).toHaveBeenCalledWith('DELETE FROM memories WHERE id = $1', [maliciousId])
    })
  })
