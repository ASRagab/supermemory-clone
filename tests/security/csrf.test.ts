...
describe('Token Reuse Prevention', () => {
  ...
  it('should enforce token expiry window', () => {
    ...
    const value = 'test-value'
    const signature = createHmac('sha256', secret).update(`${value}.${almostExpired}`).digest('hex')
...
describe('Cookie Tampering Detection', () => {
  ...
  it('should detect tampered token value', () => {
    ...
    const isValid = verifyTokenConstantTime(tamperedValue, token.signature, token.timestamp, secret)
  ...
  it('should detect tampered signature', () => {
    ...
    const isValid = verifyTokenConstantTime(token.value, tamperedSig, token.timestamp, secret)
  ...
  it('should detect tampered timestamp', () => {
    ...
    const isValid = verifyTokenConstantTime(token.value, token.signature, tamperedTimestamp, secret)
  ...
  it('should detect length mismatch for truncated signature', () => {
    ...
    const isValid = verifyTokenConstantTime(token.value, truncatedSig, token.timestamp, secret)
  ...
  it('should detect length mismatch for extended signature', () => {
    ...
    const isValid = verifyTokenConstantTime(token.value, extendedSig, token.timestamp, secret)
