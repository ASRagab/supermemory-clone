...
describe('MCP Stdio Transport CSRF Protection', () => {
  ...
  it('should bind CSRF token to process ID for stdio', () => {
    ...
    const isValidWrongPid = verifyTokenConstantTime(
      token.value,
      token.signature,
      token.timestamp,
      secret,
  ...
  it('should protect stdio with capability tokens', () => {
    ...
    capability.signature = createHmac('sha256', secret)
      .update(capabilityString)
      .digest('hex');
  ...
  it('should prevent stdio message injection', () => {
    ...
    const isInjection = !allowedTools.some((tool) =>
      injectedMessage.params.toolName === tool
    );
