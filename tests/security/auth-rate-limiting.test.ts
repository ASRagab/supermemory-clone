...
class AuthRateLimiter {
  ...
  isAllowed(identifier: string): boolean {
    ...
    entry.attempts = entry.attempts.filter((timestamp) => now - timestamp < this.config.windowMs)
  ...
  getAttemptCount(identifier: string): number {
    ...
    const now = Date.now()
    return entry.attempts.filter((timestamp) => now - timestamp < this.config.windowMs).length
  }
...
describe('Key Enumeration Prevention', () => {
  ...
  it('should prevent username enumeration via error messages', () => {
    ...
    const errors = testCases.map(({ username }) => {
      ...
      return exists ? 'Invalid username or password' : 'Invalid username or password'
    })
