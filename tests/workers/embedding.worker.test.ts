...
describe('EmbeddingWorker', () => {
  ...
  beforeEach(() => {
    ...
    vi.spyOn(embeddingServiceModule, 'getEmbeddingService').mockReturnValue(mockEmbeddingService)
    ...
    vi.spyOn(pgvectorModule, 'createPgVectorStore').mockReturnValue(mockVectorStore as any)
  })
  ...
  describe('Initialization', () => {
    ...
    it('should initialize with custom connection string', async () => {
      ...
      expect(pgvectorModule.createPgVectorStore).toHaveBeenCalledWith(customConnection, 1536, expect.any(Object))
    })
  })
  ...
  describe('Batch Processing', () => {
    ...
    it('should process chunks in batches of 100', async () => {
      ...
      mockEmbeddingService.batchEmbed.mockResolvedValue(Array(100).fill(new Array(1536).fill(0.1)))
    ...
    it('should respect custom batch size', async () => {
      ...
      mockEmbeddingService.batchEmbed.mockResolvedValue(Array(50).fill(new Array(1536).fill(0.1)))
    ...
    it('should filter out empty chunks', async () => {
      ...
      mockEmbeddingService.batchEmbed.mockResolvedValue([new Array(1536).fill(0.1), new Array(1536).fill(0.2)])
  ...
  describe('Vector Store Integration', () => {
    ...
    it('should store embeddings with correct metadata', async () => {
      ...
      mockEmbeddingService.batchEmbed.mockResolvedValue([new Array(1536).fill(0.1)])
    ...
    it('should use correct namespace for memories', async () => {
      mockVectorStore.addBatch.mockImplementation((_entries: any, options: any) => {
  ...
  describe('Factory Function', () => {
    it('should create and initialize worker', async () => {
      const { createEmbeddingWorker } = await import('../../src/workers/embedding.worker.js')
    ...
    it('should accept custom parameters', async () => {
      ...
      const newWorker = await createEmbeddingWorker('custom-queue', 'postgresql://custom:5432/test')
      expect(newWorker).toBeInstanceOf(EmbeddingWorker)
