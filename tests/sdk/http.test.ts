...
describe('HTTPClient', () => {
  ...
  describe('constructor', () => {
    ...
    it('should accept custom base URL', () => {
      ...
      expect(mockFetch).toHaveBeenCalledWith(expect.stringContaining('https://custom.api.com/test'), expect.any(Object))
    })
  ...
  describe('error handling', () => {
    ...
    it('should throw BadRequestError for 400 response', async () => {
      mockFetch.mockResolvedValueOnce(new Response(JSON.stringify({ error: 'Invalid input' }), { status: 400 }))
  ...
  describe('abort handling', () => {
    ...
    it('should throw APIUserAbortError when request is aborted by user', async () => {
      ...
      await expect(client.get('/test', { requestOptions: { signal: controller.signal } })).rejects.toThrow()
    })
  })
})
...
describe('APIPromise', () => {
  it('should resolve with parsed response', async () => {
    ...
    const promise = new APIPromise(Promise.resolve(response), async (res) => (await res.json()) as { data: string })
  ...
  it('should reject on parse error', async () => {
    ...
    const promise = new APIPromise(Promise.resolve(response), async (res) => JSON.parse(await res.text()) as unknown)
