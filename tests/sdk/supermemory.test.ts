...
describe('SuperMemory SDK', () => {
  ...
  describe('Document Methods', () => {
    ...
    describe('updateDocument', () => {
      ...
      it('should throw NotFoundError for non-existent document', async () => {
        await expect(sdk.updateDocument('non-existent', { content: 'test' })).rejects.toThrow(
          NotFoundError
        );
      });
    });
  ...
  describe('Error Handling', () => {
    describe('AuthenticationError', () => {
      it('should throw AuthenticationError for invalid API key', async () => {
        ...
        await expect(invalidSdk.createDocument({ content: 'test' })).rejects.toThrow(
          AuthenticationError
        );
      });
  ...
  describe('Edge Cases', () => {
    ...
    it('should handle concurrent requests', async () => {
      const promises = Array.from({ length: 10 }, (_, i) =>
        sdk.createDocument({ content: `Concurrent ${i}` })
      );
