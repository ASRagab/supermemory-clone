...
describe('Memories Resource', () => {
  ...
  describe('get()', () => {
    ...
    it('should URL-encode memory ID', async () => {
      ...
      expect(mockFetch).toHaveBeenCalledWith(expect.stringContaining('/v4/memories/mem%2Fspecial'), expect.any(Object))
    })
  })
  ...
  describe('list()', () => {
    ...
    it('should include filter parameters', async () => {
      mockFetch.mockResolvedValueOnce(new Response(JSON.stringify({ memories: [], pagination: {} })))
  ...
  describe('add()', () => {
    it('should add memory via POST to /v4/memories', async () => {
      mockFetch.mockResolvedValueOnce(new Response(JSON.stringify({ id: 'mem-new', status: 'created' })))
  ...
  describe('update()', () => {
    it('should update memory via PATCH', async () => {
      mockFetch.mockResolvedValueOnce(new Response(JSON.stringify({ id: 'mem-123', status: 'updated' })))
  ...
  describe('forget()', () => {
    it('should soft delete memory', async () => {
      mockFetch.mockResolvedValueOnce(new Response(JSON.stringify({ id: 'mem-123', forgotten: true })))
  ...
  describe('uploadFile()', () => {
    it('should upload file as memory', async () => {
      mockFetch.mockResolvedValueOnce(new Response(JSON.stringify({ id: 'mem-upload', status: 'processing' })))
