...
the right algorithm for your problem.
      ...
      while (!completed && (Date.now() - startTime) < timeout) {
        const queueStatus = await db.select()
          ...
          .where(eq(processingQueue.documentId, doc.id));
      ...
      const createdMemories = await db.select()
        .from(memories)
        .where(eq(memories.documentId, doc.id));
  ...
  describe('Performance Metrics', () => {
    ...
    it('should report cost for embedding operations', async () => {
      ...
      const embeddingJobs = await db.select()
        .from(processingQueue)
        .where(eq(processingQueue.stage, 'embedding'));

      const jobsWithCost = embeddingJobs.filter(job =>
        job.metadata && 'estimatedCost' in job.metadata
      );
  ...
  describe('Database State', () => {
    it('should update document status after processing', async () => {
      const processedDocs = await db.select()
        .from(documents);
    ...
    it('should maintain referential integrity', async () => {
      ...
      for (const emb of allEmbeddings) {
        const [memory] = await db.select()
          .from(memories)
          .where(eq(memories.id, emb.memoryId));
