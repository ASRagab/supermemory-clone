...
class SecurityStack {
  ...
  verifyCsrfToken(value: string, signature: string, timestamp: number, maxAge: number = 3600000): boolean {
    // Check expiry
  ...
  logAudit(level: 'info' | 'warn' | 'error', event: string, userId?: string, details: Record<string, any> = {}): void {
    this.auditLogs.push({
      timestamp: new Date(),
      level,
      event,
...
describe('Full Authentication Stack Flow', () => {
  ...
  it('should complete successful full stack flow: rate limit → CSRF → auth → authz → MCP', async () => {
    ...
    const csrfValid = stack.verifyCsrfToken(csrfToken.value, csrfToken.signature, csrfToken.timestamp)
