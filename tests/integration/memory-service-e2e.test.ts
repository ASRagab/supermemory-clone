...
describe('Memory Service E2E Integration Tests', () => {
  ...
  describe('Scenario 6: Error Handling and Rollback Scenarios', () => {
    it('should rollback all changes when memory storage fails', async () => {
      vi.spyOn(repository, 'create').mockRejectedValueOnce(new Error('Storage failure'))

      await expect(service.processAndStoreMemories('Test content for rollback.')).rejects.toThrow()
    ...
    it('should rollback when relationship storage fails', async () => {
      vi.spyOn(repository, 'createRelationshipBatch').mockRejectedValueOnce(new Error('Relationship storage failed'))
    ...
    it('should rollback when supersede update fails', async () => {
      ...
      vi.spyOn(repository, 'update').mockRejectedValueOnce(new Error('Supersede failed'))
    ...
    it('should validate container tags and reject invalid ones', async () => {
      await expect(service.processAndStoreMemories('Test content.', { containerTag: '' })).rejects.toBeInstanceOf(
        ValidationError
      )
