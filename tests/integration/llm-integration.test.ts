...
describe('OpenAI Provider Integration (Real API)', () => {
  ...
  it.skipIf(!HAS_OPENAI)(
    ...
    async () => {
      ...
      await expect(
        invalidProvider.generateJson('System prompt', 'User prompt')
    ...
    async () => {
      ...
      const promises = prompts.map((prompt) =>
        provider.generateJson(
          'Classify. Respond JSON: {"type": "fact|event|preference|skill|relationship"}',
          prompt
        )
      );
      ...
      updateStats('openai', results.reduce((sum, r) => sum + (r.tokensUsed?.total ?? 0), 0));
    },
    { timeout: 60000 }
  );
  ...
  it.skipIf(!HAS_OPENAI)(
    ...
    async () => {
      ...
      await expect(
        timeoutProvider.generateJson(
          'System prompt with lots of text to process',
          'User prompt that takes time'
        )
      ).rejects.toThrow();
    },
    { timeout: 30000 }
  ...
  it.skipIf(!HAS_OPENAI)(
    ...
    async () => {
      ...
      const result1 = await provider.generateJson(
        'Classify. Respond JSON: {"type": "fact"}',
        prompt
      );
      ...
      const result2 = await provider.generateJson(
        'Classify. Respond JSON: {"type": "fact"}',
        prompt
      );
  ...
  it.skipIf(!HAS_OPENAI && !HAS_ANTHROPIC)(
    ...
    async () => {
      ...
      const newMemory = createTestMemory(
        'I like pizza, especially margherita and pepperoni',
        'preference'
      );
  ...
  it.skipIf(!HAS_OPENAI && !HAS_ANTHROPIC)(
    ...
    async () => {
      const oldMemory = createTestMemory(
        'I like pizza, pasta, and Italian food',
        'preference'
      );
