...
/
  ...
  packages/
    install/
      package.json             # published as @supermemory/install
      src/cli.ts
...
- Proposed name: `@supermemory/runtime`
...
- Proposed name: `@supermemory/install`
...
```bash
npx -y @supermemory/install@latest <mode> [options]
```
...
3. Resolve runtime source:
   - default: npm registry tarball for `@supermemory/runtime`
...
3. smoke test `npx -y @supermemory/install@latest ...`
...
**Goal**: ship `@supermemory/install` as a thin wrapper around the canonical installer
...
- `npx -y @supermemory/install@latest agent --dir ./tmp-smoke --mcp project --source-path <local>` succeeds locally
...
1. `npx -y @supermemory/install@latest full --dir ./supermemory --mcp project` is the default recommended command.
