...
export class ProfileRepository {
  ...
  async promoteFact(containerTag: string, factId: string): Promise<ProfileFact | null> {
    ...
    const newDynamicFacts = [
      ...profile.dynamicFacts.slice(0, factIndex),
      ...profile.dynamicFacts.slice(factIndex + 1),
    ];
  ...
  private calculateRelevanceScore(fact: ProfileFact): number {
    ...
    return (
      fact.confidence * 0.3 + recencyScore * 0.3 + accessScore * 0.2 + reinforcementScore * 0.2
    );
  }
}
