...
const RELATIONSHIP_INDICATORS = {
  ...
  supersedes: [
    /\b(replaced|superseded|obsolete|deprecated|archived)\b/i,
    /\b(new version|latest|updated|revised)\b/i,
  ],
};
...
export class ContradictionDetectorService {
  ...
  getStats() {
    ...
    const contradictionRate =
      this.stats.totalChecks > 0
        ? (this.stats.contradictionsFound / this.stats.totalChecks) * 100
        : 0;
  ...
  private async detectWithLLM(
    ...
    try {
      ...
      throw new Error(
        `LLM contradiction detection failed: ${error instanceof Error ? error.message : String(error)}`
      );
    }
  }
