...
export class ProfileService {
  ...
  async updateProfile(containerTag: string, facts: ProfileFact[]): Promise<UserProfile> {
    ...
    const updated = await this.repository.updateFacts(
      containerTag,
      staticFacts,
      this.enforceDynamicLimit(dynamicFacts)
    );
  ...
  private extractFactFromSentence(sentence: string, sourceId?: string): ProfileFact | null {
    ...
    if (classification.type === 'dynamic') {
      const expirationHours =
        classification.suggestedExpirationHours ?? this.options.defaultDynamicExpirationHours;
